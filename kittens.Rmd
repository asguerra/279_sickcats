...
```{r dimensional.parameters.table, include=T}
#parameters.table <- data.frame("parameter" = c("r","K",paste($\mu$),"a","d",paste($\beta{PM}$,paste($\delta$), "definition" = "bird intrinsic growth rate","bird carrying capacity", "predation rate (cats on birds)", "conversion efficiency of cats", "cat per-capita mortality", "contact rate for proportionate mixing", "FIV virulence"), "value" = , "units" =, stringsAsFactors = FALSE)
```

```{r non.dimensional.parameters.table, include=T}
#parameters.table <- data.frame("parameter" = c("e","m",paste($\alpha$),"paste($\sigma{PM}$)"), "definition" = " ", "value" = , "units" =, stringsAsFactors = FALSE)
```
...
- lethargy is probably at play here when a cat is sick, maybe not as successful at hunting or has less contact with other cats. 
