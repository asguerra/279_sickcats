...
for(i in 2:length(t.set)){
  ...
  dB <- (B*(1-B) -B*C*(1/((I)+1)) - 0.25*B*I*C ) *dt
  dC <- (e*B*C - m*C - alpha*C*I)*dt
  dI <- (((sigma.pm - alpha)* (1-I) - e*B)*I) *dt
...
plot(t.set, B.simu3, type= "l", ylim= c(0, 1), ylab="Population (dimensionless)", xlab="time (rT)")  
lines(t.set,C.simu3,lwd=2,col='dodgerblue')
lines(t.set, I.simu3, lwd = 2, col= "coral3")
legend(x=16,y=1, col = c("black","dodgerblue","coral3"), lty=1,lwd=2,legend = c("birds", "cats", "FIV"))
text(x=3.5, y=0.04, adj=0, label="FIV introduced")
identical(I.simu3, I.simu2)
