...
In this time series we selected a biologically relevant, yet random value for e (10 in this case), from the range provided by the authors of the study. We can produce a time series with different values of e. As was done in a time series in the study, there was a lag in the introduction of FIV to the cat population. In this case we introduced FIV at time step 3. 
```{r}
B.simu1 <- NaN*t.set 
  B.simu1[1] <- .3
C.simu1 <- NaN*t.set
...
for(i in 2:length(t.set)){
  	...
  	if(t.set[i] < 3){I.simu1[i-1] <- Ino } else { 
  	  if(t.set[i] == 3) {I.simu1[i-1] <- Istart} else { I <- I.simu1[i-1]}}
...
text(x=3.4, y=0.03, labels="FIV introduced",adj=0)
